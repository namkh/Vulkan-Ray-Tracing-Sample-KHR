...
{ 
    ...
    float ndv = dot(vertexNormalW, negWorldRayDirection);
    //front face
    if(ndv > -0.1f)
    {
        ndv = dot(normal, negWorldRayDirection);
        vec3 negLightDir = normalize(-globalConstants.lightDir);
    
        float ndl = clamp(dot(normal, negLightDir), 0.0001f, 1.0f);
        ndv = clamp(ndv, 0.0001f, 1.0f);
        ...
        if(materialData.materialTypeIndex == SURFACE_TYPE_TRANSPARENT_REFRACT && payload.traceDepth < 3)
        {
            vec3 halfVec = normalize(negWorldRayDirection + negLightDir);
            float ldh = clamp(dot(negLightDir, halfVec), 0.0001f, 1.0f);
            float reflectiveIndex = pow((IOR_AIR - materialData.indexOfRefraction) / (IOR_AIR + materialData.indexOfRefraction), 2.0f);    
            vec3 F = SchlickFesnel(vec3(reflectiveIndex), ldh);
            float R = 1 - F.x;

            float refractionRatio =  payload.indexOfRefraction / materialData.indexOfRefraction;
            vec3 rayDir = normalize(refract(gl_WorldRayDirectionEXT, normal, refractionRatio));
            payload.indexOfRefraction = materialData.indexOfRefraction; 
            payload.traceDepth++;
            traceRayEXT(topLevelAs, gl_RayFlagsNoneEXT, 0xFF, 0, 0, 0, worldPos.xyz, min, rayDir, max, 0);
            payload.traceDepth--;
           
            refractColor = payload.hitColor * R;
        }
        ...
        if(materialData.materialTypeIndex == SURFACE_TYPE_TRANSPARENT_REFRACT)
        {
        ...
        //transparent
        vec3 transparentColor = vec3(0.0f);
        if(materialData.materialTypeIndex  == SURFACE_TYPE_TRANSPARENT && payload.traceDepth < 3)
        {
            payload.traceDepth++;
            traceRayEXT(topLevelAs, gl_RayFlagsCullBackFacingTrianglesEXT, 0xFF, 0, 0, 0, worldPos.xyz, min, gl_WorldRayDirectionEXT, max, 0);
            payload.traceDepth--;
            transparentColor = payload.hitColor;
        }

        if(materialData.materialTypeIndex == SURFACE_TYPE_TRANSPARENT)
        {
            payload.hitColor = mix(transparentColor, resColor, materialData.color.w * diffuse.w);
        }
        else
        {
            payload.hitColor = resColor;
        }
    }
    //back face
    else
    {
        if(materialData.materialTypeIndex == SURFACE_TYPE_TRANSPARENT_REFRACT && payload.traceDepth < 3)
        {
            float refractionRatio =  payload.indexOfRefraction / IOR_AIR;
            vec3 rayDir = normalize(refract(gl_WorldRayDirectionEXT, -normal, refractionRatio));
